...
``Tuning'' the many parameters of these flock models can be difficult, leading to research on better tools for that task. EvoFlock seeks to address this problem. It is a robust gradient-free evolutionary optimizer for parameter tuning to create, improve, or alter multi-agent motion models. It is well suited for large numbers of fast moving agents with potentially conflicting goals. It optimizes parameters toward multiple procedural objectives, allowing for complex inverse design tasks. 

An early, but not very successful, attempt at evolutionary inverse design for flock simulations \citep{reynolds_evolved_1993} used \textit{genetic programming} \citep{koza_genetic_1992}. (GP is a variation of the \textit{genetic algorithm} (GA) \citep{holland_adaptation_1975}. A modern survey of the larger field of \textit{evolutionary computation} can be found in
