                                               ...
                                               double rate = 0.5)
{
    ...
    // Were any overlaps found during current iteration? Reset for each.
    bool any_overlaps_found = true;
    // Jiggle sphere centers during relaxation process.
    auto jiggle = [](){ return EF::RS().randomPointInUnitRadiusSphere(); };
    // Generate initial random sphere centers (without regard to intersection).
    double hm = margin / 2;
    max_distance -= hm;  // Adjust for margins.
    std::vector<Sphere> spheres;
    for (auto r : radii){spheres.push_back(Sphere(jiggle()*max_distance,r+hm));}
    // If 2 given Spheres overlap: move each of them away from the other.
    auto adjust_sphere_pair = [&](Sphere& a, Sphere& b)
    {
        ...
        if (separation < 0)
        {
            double adjust_mag = -separation * rate;
            Vec3 adjust_vec = offset_from_b.normalize() * adjust_mag * 0.5;
            a.center += adjust_vec + jiggle();
            b.center -= adjust_vec + jiggle();
            any_overlaps_found = true;
        }
        // Push Sphere toward global origin if it extends past max_distance.
        auto push_toward_origin = [&](Sphere& s)
        {
            double max_r = s.center.length() + s.radius + hm;
            double excess = max_r - max_distance;
            if (excess > 0)
            {
                s.center -= (s.center.normalize() * excess * rate) + jiggle();
            }
        };
    ...
    for (int i = 0; any_overlaps_found and (i < max_iterations); i++)
    {
    ...
    // Extract sphere centers for return value.
    std::vector<Vec3> centers;
    for (auto& s : spheres) { centers.push_back(s.center) ; }
    return centers;
}
