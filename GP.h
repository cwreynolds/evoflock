                        ...
                        const LP::FunctionSet& fs)
{
    ...
    // Add a subtree for each element of FlockParameters.
...
inline Vec3 sanitizeEvolvedSteeringForce(Vec3 v)
{
    if (troublesome(v.x())) { return {}; }
    if (troublesome(v.y())) { return {}; }
    if (troublesome(v.z())) { return {}; }
    return v;
}
