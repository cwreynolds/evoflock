...
``Tuning'' the many parameters of these flock models can be difficult, leading to research on better tools for this task. EvoFlock seeks to be a robust, gradient-free, evolutionary optimizer for parameter tuning for multi-agent motion models -- especially for large numbers of fast moving agents. An early, but not very successful, attempt at inverse design for flock simulations \citep{reynolds_evolved_1993} used \textit{genetic programming} \citep{koza_genetic_1992}. (GP is a variation of the \textit{genetic algorithm} (GA) \citep{holland_adaptation_1975}. A modern survey of the larger field of \textit{evolutionary computation} can be found in
...
% Wahrman(?)
% flocking friends?
\textbf{[[[remember to remove Acknowledgements for review.]]] [[[Full Papers are 3 to 8 pages long (not including references or acknowledgments)]]]}
